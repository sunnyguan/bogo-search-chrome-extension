...
function waitForElmChange(selector, val) {
  return new Promise((resolve) => {
    if (
      ...
      document.querySelector(selector).getAttribute("href") !== val
    ) {
      return resolve(document.querySelector(selector));
    }
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  if (request.type === "room_info") {
    if ("players" in request.data) {
      document.querySelector("#room-size").innerHTML =
        "<b>Players: </b>" + request.data.players.join(", ");
    }
    ...
    if ("room_id" in request.data) {
      ...
      document.querySelector("#room-name").textContent =
        request.data.room_name + " (id=" + request.data.room_id + ")";
    }
    ...
    if ("is_started" in request.data) {
      if (request.data.is_started) startRoom(false);
      else started = false;
    }
    ...
    for (const ranking of response.rankings) {
      ...
      for (const status of response.question_status[ranking[0]]) {
        ...
        console.log(status);
        if (status[0] === 2) {
          show = `<span style="color:black">${secToHMS(
            Math.floor(status[1])
          )} ✅</span>`;
        } else if (status[0] === 1) {
          show = "❌";
